...
// CORS configuration - allows same-origin (when hosted together) and Vercel preview deployments
app.use((req, res, next) => {
  ...
  if (!origin || origin.includes(host)) {
    ...
    // Allow specific origins for cross-origin (preview deployments, etc.)
...
console.log('ENV_DEBUG ' + JSON.stringify({
  hasUser: !!process.env.PGUSER,
  hasHost: !!process.env.PGHOST,
  hasDatabase: !!process.env.PGDATABASE,
  hasPassword: !!process.env.PGPASSWORD,
  hasPort: !!process.env.PGPORT,
...
// FORGOT PASSWORD ENDPOINTS - Moved under /api for consistency
...
//LOGIN AND SIGNUP - Moved under /api for consistency
app.post("/api/signup",async(req,res)=>{
    console.log('HANDLER_DEBUG signup start ' + req.body?.email);
...
app.post("/api/login",async(req,res)=>{
  console.log('HANDLER_DEBUG login start ' + req.body?.email);
  const {email,password}=req.body;
  try{
    const user=await db.query("SELECT * FROM users WHERE email=$1",[email]);
    if(user.rows.length==0){
